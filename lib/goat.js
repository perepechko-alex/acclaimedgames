...
function writeToDb(batch) {
  ...
  let sql = `INSERT OR REPLACE INTO finalResults(rank, name, totalscore) VALUES ${placeholders}`;
  const flatBatch = batch.flat();
  db.run(sql, ...flatBatch, (err) => {
...
export function populateFinalResults(gameArr) {
  ...
  for (let game of gameArr) {
    batch.push([game.finalRank, game.name, game.totalScore]);
    if (batch.length === 100) {
      writeToDb(batch);
      batch = [];
    }
