...
export async function getGoat() {
  ...
  return new Promise((resolve, reject) => {
    db.each(
      `SELECT name, SUM(weightedpoints) weightedpoints, COUNT(*) numoflists
            FROM goat
            GROUP BY name`,
      [],
      (err, row) => {
...
function writeToDb(batch) {
  ...
  const sql = db.prepare(
    `INSERT OR REPLACE INTO finalResults(rank, name, totalscore, numoflists) VALUES ${placeholders}`
  );
...
export function populateFinalResults(gameArr) {
  ...
  for (let game of gameArr) {
    batch.push([game.finalRank, game.name, game.totalScore, game.numberOfLists]);
    if (batch.length === 100) {
      writeToDb(batch);
      batch = [];
    }
