...
export function populateTemp() {
  db.serialize(() => {
    db.run(
      `INSERT INTO allTemp
            SELECT a.name, a.weightedpoints
            from goat a
            UNION ALL
            SELECT b.name, b.weightedpoints
            from goty b
            ORDER BY a.name`
    );
  });
...
export function populateFinalResults(gameArr) {
  let sql = `INSERT INTO finalResults(rank, name, totalScore) VALUES (?, ?, ?)`;
...
export async function getOtherGameVersions() {
  let queryVars = otherGames.map((game) => "(?) ").join("OR name = ");
  let sql = `SELECT name, SUM(weightedpoints) total FROM allTemp WHERE name = ${queryVars} GROUP BY name`;
  return new Promise((resolve, reject) => {
  ...
  for (let game of gamesArr) {
    for (let other of otherGames) {
      ...
      if (
        ...
        game.totalScore += other.total;
      if (
        other.name === "Counter-Strike: Source" &&
