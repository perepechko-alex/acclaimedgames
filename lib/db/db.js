...
export function createViews() {
  db.serialize(() => {
    db.run(`CREATE VIEW IF NOT EXISTS results_with_metadata AS
      SELECT finalResults.rank, finalResults.name, finalResults.totalScore, finalResults.numOfLists, game_metadata.releasedate, game_metadata.platforms, game_metadata.developers FROM finalResults
    ...
    db.run(`CREATE VIEW IF NOT EXISTS results_with_metadata_1000 AS
      SELECT finalResults.rank, finalResults.name, finalResults.totalScore, finalResults.numOfLists, game_metadata.releasedate, game_metadata.platforms, game_metadata.developers FROM finalResults
      INNER JOIN game_metadata ON finalResults.name = game_metadata.name
      ORDER BY CASE WHEN finalResults.rank IS NULL THEN 1 ELSE 0 END, finalResults.rank LIMIT 1000`);
  });
}
